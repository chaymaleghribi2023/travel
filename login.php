...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($row['accounttype'] == 'user') {
            ...
            header('location:userpage.php'); // Redirige vers la page de l'utilisateur
            exit; // Termine le script après la redirection
        } elseif ($row['accounttype'] == 'admin') {
            $_SESSION['admin_name'] = $row['username'];
            header('location:admin.php'); // Redirige vers la page de l'administrateur
            exit; // Termine le script après la redirection
        } else {
...
<a href="home.php" class="logo">agence grira travel. <img src="image/logo.jpg" alt="Travel Logo" width="20px" height="20px"></a>
    ...
    </section>   <div class="heading" style="background:url(image/chi.webp) no-repeat">
    <h1>login</h1>   
</div>
   <div class="wrapper">
...
<div class="logreg-link">
    <p>don't have an account?<a href="#" class="register-linlk">signup</a></p>
</div>
    </form>
    </div>
